...
def login(request: Request,
          username: str = Body(...),
          passwd: str = Body(...), ):
    logger.debug(f"User {username} logging in...")
    session = new_session()
...
def register(request: Request,
    ...
    user = session.query(Users).filter(or_(Users.username == username,
                                           Users.phone == (phone or '$$$$$$'),  # non-existent string used as condition
...
@logger.catch()
@route.post("/_find_pwd", summary='Retrieve Password')
def find_pwd(request: Request,
    ...
    is_send = send_email("Retrieve Password", f"Retrieve your password via the link below! \r\n {url}", email)
    return success("Email sent, please retrieve your password via email!") if is_send else error("Failed to send email!")


@logger.catch()
@route.post("/_find_modify_pwd", summary='Retrieve and Modify Password')
def find_modify_pwd(request: Request,
    ...
    check_true(new_pwd == repeat_new_pwd, "Please confirm that the entered passwords match")
...
class CurrentUser:
    def __init__(self, request: Request, token: str = Header(None)):
        ...
        if not users:
            raise ServerError("Exception in token validation")
